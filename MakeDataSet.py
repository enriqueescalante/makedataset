...
name_models_from_orbifolder = "Z2xZ4_models_SP_5.txt"
name_models_writted = "modeltest.txt"
name_geometry = "./Geometry/Geometry_Z2xZ4_1_1.txt"
name_dataset = "datasetZ2xZ4_SEC[6].csv"
...
# Return a list with N's for global verification
def __local__(model, sectors_considered, nomassive):
...
def make_dataset(list_sectors, start,creategaugegroups=True):

    MOD = open(name_models_writted, "r")  # File bridge

    sectors_considered = load_sectors(list_sectors)
    nomassive = no_massives_states()
    ...
    if start == 0:
        ...
        for sector in sectors_considered:
            ...
            for fixed in sector:
                for i in range(1, 3):
                    head.append("N"+str(sec)+str(fix)+str(i))
                fix = fix + 1
            fix = 1
            sec = sec + 1
        DS.write(",".join(head)+"\n")
    ...
    for model in MOD:
        ...
        if conta > start:
            ...
            Ns.extend(__local__(model, sectors_considered, nomassive))
            DS.write(str(Ns)[1:-1]+"\n")
        else:
            pass
        conta = conta + 1
...
#numU1(name_models_from_orbifolder)
#write_parameters()
make_dataset(list_sectors, start, False)
